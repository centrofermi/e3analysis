...
// e3RunDbConn GetTelescopeList member function
//------------------------------------------------------------
int e3RunDbConn::GetTelescopeList(vector<string>& telIDList, const string whereClausesStr){
  ...
  _queryStr<<"SELECT name FROM telescopes ";
  ...
  while ((_dbRow = mysql_fetch_row(_queryRes))) { 
      ...
      if(_vLevel>5){
  	cout<<"\t\t"<<(_dbRow[0]  ? _dbRow[0]:"NULL")<<endl;
  	irow++;
      }

      telIDList.push_back(_dbRow[0]);
...
// e3RunDbConn GetDaqConfList member function
//------------------------------------------------------------
int e3RunDbConn::GetDaqConfList(vector<string>& parValueList, const string stationID, const string tWinLowStr, const string tWinUpStr, const string colNameList){

  //******************************
  //Time window limit in sec
  //******************************
  char dummy;
  stringstream lineStream;
  ...
  lineStream >> _tWinLow.tm_year >> dummy >> _tWinLow.tm_mon >> dummy >> _tWinLow.tm_mday >> dummy >> _tWinLow.tm_hour >> dummy >> _tWinLow.tm_min >> dummy >> _tWinLow.tm_sec;
  if(!lineStream && (_tWinLow.tm_year==0 || _tWinLow.tm_mon==0 || _tWinLow.tm_mday==0)) {
    cerr << "[e3RunDbConn::GetDaqConfList - ERROR] You entered an invalid date/time value." << endl;
    return 1;
  }
  ...
  _queryStr.clear();
  _queryStr.precision(0);
  _queryStr<<"SELECT "<<colNameList<<",valid_from,valid_until FROM daq_configurations ";
  ...
  _queryStr<<"AND valid_from>='"<<tWinLowStr<<"' and valid_until<'"<<tWinUpStr<<"';";

  if(_vLevel>1) cout<<"[e3RunDbConn::GetDaqConfList - INFO] Performing query: "<<_queryStr.str()<<endl;

...
void doCoincCERN_01_02(const char *fileIn="coincCERN_0102.root"){
  ...
  hDeltaPhiBack->Sumw2();
  hThetaRelBack->Sumw2();
  hDeltaThetaBack->Scale(0.01);
  hDeltaPhiBack->Scale(0.01);
  hThetaRelBack->Scale(0.01);
  hAngleBack->Scale(0.01);
  ...
  ff->SetParLimits(2,20,400); // fix witdh if needed
  ...
  TPaveText *text = new TPaveText(150,(h->GetMinimum()+(h->GetMaximum()-h->GetMinimum())*0.6),950,h->GetMaximum());
  text->SetFillColor(0);
